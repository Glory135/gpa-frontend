...
const SavedBreadcrumb = ({ darkMode, setSemester, setlvl }) => {
	...
	const handleCgpaCalc = () => {
		...
		for (let i = 0; i <= allData.length; i++) {
			for (let j = 0; j < allData[i]?.semesters.length; j++) {
				semesters.push(allData[i].semesters[j]);
			}
		}
		const res = calCGP(semesters);
		setCgpa(res);
	};
				...
				{allData.length >= 1 ? (
					<>
								...
								return (
									<div
										...
										<div
											...
											<div className='gp'>
												{singlelvl.GP.toFixed(2)} GPA
											</div>
										</div>
										<div
											onClick={() =>
												handleDelete(singlelvl.lvl)
											}
											className='second'>
											<HighlightOffIcon />
										</div>
						...
						<div className='btn-cgpa-res'>
							<button
								onClick={() => handleCgpaCalc()}
								className='btn'>
								Calculate CGPA For Saved Data
							</button>
							{cgpa && <span>CGPA: {cgpa.toFixed(2)}</span>}
						</div>
					</>
				) : (
					<div className='no_data'>No Saved Data!!!</div>
				)}
			</div>
		</div>
	);
